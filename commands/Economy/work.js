...
module.exports.data = new SlashCommandBuilder()
    ...
    .addIntegerOption(option =>
        option
            .setName('amount')
            .setDescription('The amount of mana you want to use.')
            .setRequired(false))
    .setDMPermission(true)

module.exports.run = async (client, interaction) => {
    const stats = inv.prepare("SELECT * FROM stats WHERE id = ?;").get(interaction?.member.user.id ?? interaction.user.id);
    const setStats = inv.prepare(
        "INSERT OR REPLACE INTO stats (id, user, money, mana, maxmana, business, level, xp) VALUES (@id, @user, @money, @mana, @maxmana, @business, @level, @xp);"
    );
    ...
    if (!stats) {
        ...
        interaction.channel.send(`You've just created your own profile!`)
        work(1)
    }
